...
use shared_gcp::rl::{append_rl_pair, RlPair};
...
Read the attached HTML content and return a JSON object matching the required schema.\n\
    ...
    fn handle(
        ...
        async move {
            if let Some(data) = event.data {
                ...
                let mut reader = ZipFileReader::new(cursor.compat())
                    .await
                ...
                for i in 0..reader.file().entries().len() {
                    ...
                    if filename.ends_with(".html") || filename.ends_with(".htm") {
                        ...
                        entry_reader
                            .read_to_string_checked(&mut html_content)
                            .await

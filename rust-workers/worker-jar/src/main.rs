...
Read the attached decompiled Java code and return a JSON object matching the required schema.\n\
...
impl GeminiClient {
    ...
    async fn analyze_text(
        ...
        let candidate_text = response
            .first_text_part()
            .map(str::to_owned)
            .ok_or("Vertex AI returned no candidate text.".to_string())?;
        ...
        async move {
            if let Some(data) = event.data {
                ...
                if let Some(rl_pair) = analysis.rl_pair {
                    shared_gcp::rl::append_rl_pair(&rl_pair).await.map_err(|e| format!("RL append error: {}", e))?;
                }
